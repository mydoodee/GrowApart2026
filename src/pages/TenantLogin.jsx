...
import { Mail, Lock, Eye, EyeOff, Loader2, User as UserIcon, Building, Phone } from 'lucide-react';
...
export default function TenantLogin({ user }) {
    ...
    const aptId = searchParams.get('aptId') || localStorage.getItem('activeApartmentId');
    ...
    return (
        <div className="min-h-screen bg-brand-bg flex items-center justify-center p-4 relative overflow-hidden">
                ...
                <div className="text-center mb-8">
                    <div className="w-16 h-16 bg-brand-orange-500/20 rounded-2xl flex items-center justify-center text-brand-orange-500 mx-auto mb-4 border border-brand-orange-500/30 overflow-hidden">
                        {aptBranding?.general?.logoURL ? (
                            <img src={aptBranding.general.logoURL} alt={aptBranding.general.name} className="w-full h-full object-cover" />
                        ) : (
                            <UserIcon className="w-8 h-8" />
                        )}
                    </div>
                    <h1 className="text-2xl font-bold text-white mb-1 tracking-tight">
                        {aptBranding?.general?.name || (step === 'LOGIN' ? 'เข้าสู่ระบบผู้เช่า' : 'ลืมรหัสผ่าน')}
                    </h1>
                    <p className="text-brand-gray-500 text-xs font-bold uppercase tracking-widest opacity-80">
                        {aptBranding?.general?.name ? (step === 'LOGIN' ? 'TENANT LOGIN' : 'RECOVER ACCESS') : (step === 'LOGIN' ? 'TENANT LOGIN' : 'RECOVER ACCESS')}
                    </p>
                </div>
                ...
                {step === 'LOGIN' ? (
                    <form onSubmit={handleLogin} className="space-y-5">
                        <div className="flex bg-brand-bg rounded-xl p-1 border border-white/5">
                            <button type="button" onClick={() => setAuthMethod('phone')} className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all ${authMethod === 'phone' ? 'bg-brand-card text-brand-orange-500 shadow-sm' : 'text-brand-gray-400'}`}>เบอร์โทรศัพท์</button>
                            <button type="button" onClick={() => setAuthMethod('email')} className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all ${authMethod === 'email' ? 'bg-brand-card text-brand-orange-500 shadow-sm' : 'text-brand-gray-400'}`}>อีเมล</button>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative group">
                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-brand-orange-500 transition-colors">
                                    {authMethod === 'phone' ? <Phone size={16} /> : <Mail size={16} />}
                                </div>
                                <input
                                    type={authMethod === 'phone' ? "tel" : "email"}
                                    placeholder={authMethod === 'phone' ? "08X-XXX-XXXX" : "example@mail.com"}
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between items-center px-1">
                                <label className="text-[10px] font-bold text-brand-gray-500 uppercase tracking-widest">รหัสผ่าน</label>
                                <button type="button" onClick={() => setStep('FORGOT_PASSWORD')} className="text-[10px] font-bold text-brand-gray-600 hover:text-white transition-colors">ลืมรหัสหรือไม่?</button>
                            </div>
                            <div className="relative group">
                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-brand-orange-500 transition-colors">
                                    <Lock size={16} />
                        ...
                        <div className="flex items-center gap-4 py-2">
                            ...
                            <span className="text-[8px] font-bold text-brand-gray-600 uppercase tracking-[0.2em]">OR LOGIN WITH</span>
                            <div className="flex-1 h-px bg-white/5"></div>
                        </div>
                    ...
                    <form onSubmit={handleResetPassword} className="space-y-6 animate-in slide-in-from-right duration-300">
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-brand-gray-500 uppercase px-1 tracking-widest">อีเมลกู้คืน</label>
                            <input
                                type="email"
            ...
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 text-center space-y-2 opacity-50 hover:opacity-100 transition-opacity">
                <p className="text-[10px] font-bold text-brand-gray-500 uppercase tracking-[0.2em]">GROWAPART SYSTEM</p>
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/login')} className="text-[9px] font-bold text-brand-gray-500 hover:text-brand-orange-500 transition-colors uppercase">OWNER PORTAL</button>
                    <div className="w-1 h-1 bg-white/10 rounded-full"></div>
                    <button className="text-[9px] font-bold text-brand-gray-500 hover:text-white transition-colors uppercase">SUPPORT</button>
                </div>
            </div>
        </div>
    );
