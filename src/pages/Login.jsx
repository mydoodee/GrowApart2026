...
export default function Login({ user }) {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        if (!email || !password) return setError('กรุณากรอกอีเมล/เบอร์โทรและรหัสผ่าน');
        setError('');
        setLoading(true);
        try {
            ...
            if (err.code === 'auth/invalid-credential') {
                setError('อีเมล/เบอร์โทร หรือรหัสผ่านไม่ถูกต้อง (หรือยังไม่ได้สมัครสมาชิก)');
            } else if (err.code === 'auth/operation-not-allowed') {
                setError('กรุณาเปิดการใช้งาน Email/Password Provider ใน Firebase Console');
            } else {
    ...
    const handleRegister = async (e) => {
        e.preventDefault();
        if (!name || !email || !password) return setError('กรุณากรอกข้อมูลให้ครบถ้วน');
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            setError('เราได้ส่งลิงก์รีเซ็ตรหัสผ่านไปยังอีเมลของคุณแล้ว (หากไม่พบ กรุณาเช็คใน Spam)');
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="min-h-screen bg-brand-bg flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23F39C12' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }}></div>
            ...
            <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-brand-orange-500/10 rounded-full blur-[120px] animate-pulse pointer-events-none" style={{ animationDelay: '2s' }}></div>
            ...
            <div className="w-full max-w-[360px] bg-brand-card/80 backdrop-blur-xl border border-white/10 rounded-xl px-7 py-9 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] relative z-10 transition-all">
                ...
                <div className="text-center mb-5">
                    <h1 className="text-xl font-bold text-white mb-1 tracking-tight">{getHeaderTitle()}</h1>
                    <p className="text-brand-gray-400 text-sm font-medium opacity-80">{getHeaderSubtitle()}</p>
                </div>
                ...
                {step === 'LOGIN' && (
                    <form onSubmit={handleLogin} className="space-y-4">
                        <div>
                            <label className="block text-brand-orange-500 text-[11px] font-bold mb-1.5 ml-1 uppercase tracking-wider">อีเมล / เบอร์โทรศัพท์</label>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5 px-1">
                                <label className="block text-brand-orange-500 text-[11px] font-bold uppercase tracking-wider">รหัสผ่าน</label>
                                <button
                                    type="button"
                        ...
                        <div className="flex items-center my-6">
                            ...
                            <span className="px-4 text-xs text-gray-500 font-medium">หรือดำเนินการต่อด้วย</span>
                            <div className="flex-1 border-b border-gray-800"></div>
                        </div>
                        ...
                        <div className="text-center mt-10 space-y-3">
                            <p className="text-xs text-gray-500 font-medium tracking-tight">
                                ยังไม่มีบัญชี? <button
                                    type="button"
                                    onClick={() => { setStep('REGISTER'); setError(''); setEmail(''); setPassword(''); }}
                            ...
                            <div className="pt-2 border-t border-white/5">
                                ...
                                >เข้าสู่ระบบสำหรับผู้เช่า</button>
                            </div>
                        </div>
                    </form>
                )}
                ...
                {step === 'FORGOT_PASSWORD' && (
                    <form onSubmit={handleResetPassword} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-brand-orange-500 text-[11px] font-bold mb-1.5 ml-1 uppercase tracking-wider">อีเมล</label>
                            <div className="flex items-center bg-brand-input-bg rounded-xl px-4 py-2 border border-transparent focus-within:border-brand-orange-500/50 transition-all shadow-inner">
                        ...
                        <div className="text-center mt-6">
                            ...
                            >กลับไปหน้า <span className="text-brand-orange-500 font-bold">เข้าสู่ระบบ</span></button>
                        </div>
                    </form>
                )}
                ...
                {step === 'REGISTER' && (
                    <form onSubmit={handleRegister} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-brand-orange-500 text-[11px] font-bold mb-1.5 ml-1 uppercase tracking-wider">ตั้งรหัสผ่าน</label>
                            <div className="flex items-center bg-brand-input-bg rounded-xl px-4 py-2 border border-transparent focus-within:border-brand-orange-500/50 transition-all shadow-inner">
                                <Lock className="w-4.5 h-4.5 text-brand-bg/40 mr-3 shrink-0" />
                                <input
                                    type={showPassword ? "text" : "password"}
                        ...
                        >
                            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'ยืนยันการสมัครสมาชิก'}
                        </button>
                        <div className="text-center mt-10">
                            <button
                                ...
                                className="text-gray-500 text-xs font-medium hover:text-white transition-colors"
                            >มีบัญชีอยู่แล้ว? <span className="text-brand-orange-500 font-bold">เข้าสู่ระบบ</span></button>
                        </div>
                    </form>
                )}
            </div>
